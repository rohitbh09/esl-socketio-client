...
Connection.prototype.send = function( command, args, type, cb ) {
    ...
    try {
      ...
      if(cb && typeof cb === 'function')
        this._setCallBack( requestId, cb);

      this._socketSend( request,requestId);
    }
    catch(e) {
        self.emit('error', e);
    }
...
Connection.prototype._socketSend = function( request, requestId ) {
  try {
    this.socket.emit('elsRequest', request, ( error, succes ) => {
      console.log(error, succes); // data will be 'woot'
      if( error )
        this._getCallBack( requestId, "-ERR " + error )
    });
  } catch(ex){
    this._getCallBack( requestId,"-ERR ESL ERROR")
  }
}
...
Connection.prototype._onEslReponse = function( event ) {
  console.log('_onEslReponse', event)
  var body    = event.body || "-ERR NO RESPONSE";
