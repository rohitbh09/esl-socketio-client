...
Connection.prototype.send = function( command, args, type, cb ) {
    ...
    if( typeof command != "object" ){
        ...
        if(cb && typeof cb === 'function')
            cb(self._errorResponse(response));
        return;
    }
...
Connection.prototype._socketSend = function( request, requestId ) {
  ...
  try {
    ...
    esl._debug("ESL CONN:: _socketSend error", ex);
...
Connection.prototype._onEslReponse = function( event ) {
  ...
  if( event.response ){
    var body          = event.response.body || "-ERR NO RESPONSE",
        headers       = this._parseHeader(event.response),
        eventResponse = new Event(headers, body);
    esl._debug('_onEslReponse response done')
  } else {
    var eventResponse  = this._errorResponse("-ERR NO RESPONSE");
    esl._debug('_onEslReponse no response')
  }
  esl._debug('_onEslReponse', eventResponse)
  this._getCallBack( event.requestId, eventResponse );
}

Connection.prototype._onEvent = function( event ) {
  esl._debug('_onEvent', event)
  var headers = this._parseHeader(event);
  var event = new Event(headers);
  esl._debug('_onEvent', event)
  var body    = event.body || "";
...
Connection.prototype._parseHeader = function( event ) {
  var headers = {};
  if( event && event.headers && typeof event.headers === "object" ) {
...
Connection.prototype._errorResponse = function( body, headers ) {
 ...
 var  body          = body,
      headers       = headers || [],
      eventResponse = new Event(headers, body);
  esl._debug('_errorResponse', eventResponse)
